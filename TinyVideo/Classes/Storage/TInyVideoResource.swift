...
public struct VideoDiskCache:FileCache{
    public init(identify: String) {
        ...
        if let ida  = identify.data(using: .utf8){
            self.identify = TinyHash.md5(data: ida).hexString
        }else{
...
public class TinyVideoResourceManager:NSObject,URLSessionDataDelegate{
    ...
    public func download(url:URL,indx:Int,identify:String,callback: @escaping (URL?)->Void){
        ...
        if old == nil{
            old = TinyVideoResource<VideoDiskCache>(url: url,identify: identify,queue: self.queue)
        }
    ...
    @objc public func download(url:URL,identidy:String,callback: @escaping (URL?)->Void){
        ...
        var old = self.loader[url]
        if old == nil{
            old = TinyVideoResource<VideoDiskCache>(url: url,identify: identidy,queue: self.queue)
        }
        guard let a = old else {
            callback(nil)
            return
